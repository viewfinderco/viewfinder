...
(function($, namespace) {
  ...
  var ConversationBodyView = ViewfinderView.extend({
    ...
    render : function () {
      ...
      if (this.model.loaded()) { 
        ...
        if (viewfinder.flags.isRegistered) {
          var addCommentModel = new viewfinder.models.AddCommentModel({
            viewpoint : this.model
          });
          this.appendChild(AddCommentDialogView, { model : addCommentModel, viewpoint : this.model });
          _.defer(_.bind(this.scrollToLastViewed, this));
        } else {
          this.appendChild(SignupCtaView);
        }
      } else {
    ...
    scrollToLastViewed : function () {
      var $unviewed = this.$('.unviewed-item');
      if ($unviewed.length > 0) {
        $(window).scrollTop($unviewed.first().offset().top - 100);
      } else {
  ...
  var InboxView = ViewfinderView.extend({
    ...
    initialize : function (options) {
      ...
      var imgsrc = this.prospective ?
            '/static/images/prospective_dashboard.jpg' :
            '/static/images/default_dashboard.jpg';
  ...
  var RequestArchiveGetEmailView = FormDialogBaseView.extend({
    className : 'auth-form request-archive-email',

    templateName : 'request-archive-email',

    formSchema : {
      email : { editorAttrs : { placeholder : 'Email Address' }, 
                template : FormFieldTemplate, 
                dataType : 'email' }
    },
  ...
  var RequestArchiveCompleteView = DialogBaseView.extend({
    className : 'auth-form request-archive-complete',

    templateName : 'request-archive-complete',
  ...
  var RequestArchiveView = ViewfinderView.extend({
    className : 'request-archive',

    initialize : function (options) {
      this.settings = options.settings;
      this.model = new viewfinder.models.RequestArchiveModel({
        email : viewfinder.own_user.email || null
      });
      this.bindModel('change:state', this.render);
    },
    ...
    render : function () {
      ...
      if (state == 'get_email') {
        this.appendChild(RequestArchiveGetEmailView, {model : this.model});
      } else if (state == 'complete') {
        this.appendChild(RequestArchiveCompleteView, {model : this.model});
      } else {
        this.settings.dialog(null);
      }
    }
  ...
  var DialogOverlayView = ViewfinderView.extend({
    ...
    viewMap : {
      ...
      'request-archive' : RequestArchiveView
    },
  ...
  var HeaderView = namespace.HeaderView = ViewfinderView.extend({
    ...
    render : function () {
      ...
      this.assignChild('.menu', DropdownMenuView);
  ...
  var ClosingView = namespace.ClosingView = ViewfinderView.extend({
    events : {
      'click' : 'hideBar'
    },
    ...
    render : function () {
      ...
      if (this.settings.page() === 'settings') {
        this.hideBar();
      }
    ...
    renderMobile : function () {
      this.$el.html(this.template('closing-mobile')());
    },

    renderOneCol : function () {
      this.$el.html(this.template('closing')());
    },
    ...
    hideBar : function () {
      ...
      this.settings.page('settings');
      this.app.$el.removeClass('notification');
    }
  });

  var SettingsView = namespace.SettingsView = ViewfinderView.extend({
    events : {
      'click .option-button' : 'requestArchive'
    },
    ...
    renderTwoCol : function () {
      ...
      if (this.prospective) {
        ...
        this.assignChild('.photo', CroppedPhotoView, {model : this.bgModel, containerAspect : 1.25});
        var own_user = this.scope.users.get(viewfinder.own_user.user_id);
        this.assignChild('.account-info', AccountInfoView, { model : own_user });
      }
    },

    requestArchive : function () {
      this.model.dialog('request-archive');
    }
  });
  ...
  var AppView = namespace.AppView = ViewfinderView.extend({
    ...
    initialize: function() {
      ...
      var settingsView = SettingsView.create({model : this.settings, scope: scope, el : $settings}, this);
      settingsView.render();
      ...
      var $closing = $('#closing');
      var closingView = ClosingView.create({settings : this.settings, app : this,  el : $closing}, this);
      closingView.render();
