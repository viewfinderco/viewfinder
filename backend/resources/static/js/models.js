...
(function($, namespace){ 
  ...
  var ViewpointModel = namespace.ViewpointModel = ViewfinderDataModel.extend({
    ...
    metadataKeys : [
      ...
      'viewed_seq', 'type', 'user_id', 'update_seq', 'sharing_user_id', 'last_updated'
    ],
    ...
    setViewed : function () {
      ...
      if (update_seq > viewed_seq) {
        ...
        if (viewfinder.flags.isRegistered) {
          viewfinder.query.updateViewedSeq(this.id, update_seq);
        }
      }
    },
  ...
  var RequestArchiveModel = namespace.RequestArchiveModel = Backbone.Model.extend({
    defaults : {
      email : null,
      state : 'get_email'
    },
    ...
    validate : function (attributes) {
      var errors = {};
      if (!attributes.email || attributes.email.length === 0) {
        errors.email = 'You must specify an e-mail address.';
      } else if (!isEmail(attributes.email)) {
        errors.email ='You must specify a valid e-mail address.';
      }
    ...
    proceed : function () {
      ...
      viewfinder.account.requestArchive(this.get('email'))
      .done(function (response) {
